    ...
    public void onBlockPlace(EntityChangeBlockEvent event) {
        ...
        if (blockEntity.getBlockData().getMaterial().equals(Material.SPAWNER)) {
            ...
            if (block.getState() instanceof CreatureSpawner spawner) {
                EntityType type = EntityType.values()[new Random().nextInt(EntityType.values().length)];
                if (type.equals(EntityType.PLAYER) || type.equals(EntityType.ENDER_DRAGON)) return;
                spawner.setSpawnedType(EntityType.values()[new Random().nextInt(EntityType.values().length)]);
                spawner.update();
            }
            return;
        }
